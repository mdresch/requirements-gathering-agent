...
const contextManager_1 = require("../src/modules/contextManager");
const directContextInjectionExample_1 = require("../src/examples/directContextInjectionExample");
async function testDirectContextInjection() {
    ...
    try {
        ...
        console.log('\n🔬 Test 2: Direct Context Injection');
        const injectedCount = await contextManager.injectHighRelevanceMarkdownFiles(projectPath, 60, // Lower threshold for testing
        5 // Limit to 5 files
        );
        ...
        for (const docType of testDocTypes) {
            ...
            const hasInjectedContent = stats.injectedKeys.some(key => context.includes(key) || context.includes('Injected Document'));
        ...
        await (0, directContextInjectionExample_1.runAllDirectContextInjectionExamples)(projectPath);
