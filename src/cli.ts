...
async function main() {
  try {
    ...
    const getNumericValue = (flag: string, defaultValue: number): number => {
      ...
      format: getArgValue('--format', 'markdown', ['markdown', 'json', 'yaml', 'docx', 'pptx']) as 'markdown' | 'json' | 'yaml' | 'docx' | 'pptx',
      ...
      provider: getArgValue('--provider', '', ['', 'github-ai', 'google-ai', 'azure-openai', 'azure-openai-key', 'ollama']) as '' | 'github-ai' | 'google-ai' | 'azure-openai' | 'azure-openai-key' | 'ollama'
    };// Show version
...
- Azure cloud deployment
    ...
    // Strategic statements flag support
    const generateStrategic = args.includes('--generate-strategic') || args.includes('--generate-strategic-statements');
    if (generateStrategic) generateTypes.add('strategic');
    try {
      ...
      if (generateAll || generateTypes.has('strategic')) {
        if (!options.quiet) console.log('🎯 Generating strategic statements...');
        const generator = new DocumentGenerator(context, {
          ...generationOptions,
          includeCategories: ['strategic-statements']
        });
        await generator.generateAll();
      }

      // Direct flag for stakeholder (legacy)
      ...
      // Direct flag for strategic statements
      if (generateStrategic) {
        console.log('🎯 Generating strategic statements...');
        const results = await generateDocumentsWithRetry(context, {
            includeCategories: ['strategic-statements'],
            format: options.format,
            outputDir: options.outputDir,
        ...
        if (results?.success) {
            console.log(`✅ Successfully generated ${results.generatedFiles?.length || 0} strategic statement documents`);
            console.log(`📁 Check the ${options.outputDir}/strategic-statements/ directory`);
        } else {
            console.error('❌ Failed to generate strategic statement documents');
            process.exit(1);
        }
        return;
      }
...
async function validateEnvironment(options?: { provider?: string }): Promise<boolean> {
  // Check for help flag
...
DOCUMENT TYPES:
  ...
  --generate-strategic    Generate strategic statements (mission, vision, purpose)
  --generate-strategic-statements  Generate strategic statements (alias)
 ...
 */
async function validateSpecificProvider(providerName: string): Promise<boolean> {
  console.log(`🎯 Validating selected provider: ${providerName}`);
  
  // Map CLI provider names to check functions
  const providerValidators: Record<string, () => boolean> = {
    'github-ai': () => !!(
      process.env.GITHUB_TOKEN && 
      process.env.GITHUB_ENDPOINT?.includes('models.github.ai')
    ),
    ...
    'azure-openai-key': () => !!(
      (process.env.AZURE_AI_ENDPOINT?.includes('openai.azure.com') || 
 ...
 */
async function provideProviderSpecificGuidance(providerName: string): Promise<void> {
