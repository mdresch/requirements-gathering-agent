...
import { ControlscopeprocessTemplate } from '../pmbok/ControlscopeprocessTemplate.js';
...
/**
 * Processor for the Controlscopeprocess document.
 */
export class ControlscopeprocessProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Controlscopeprocess',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Controlscopeprocess processing:', error.message);
        throw new Error(`Failed to generate Controlscopeprocess: ${error.message}`);
      } else {
        console.error('Unexpected error in Controlscopeprocess processing:', error);
        throw new Error('An unexpected error occurred while generating Controlscopeprocess');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new ControlscopeprocessTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Controlscopeprocess document.
