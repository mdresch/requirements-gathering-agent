...
import { CloseprojectphaseprocessTemplate } from '../pmbok/CloseprojectphaseprocessTemplate.js';
...
/**
 * Processor for the Closeprojectphaseprocess document.
 */
export class CloseprojectphaseprocessProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Closeprojectphaseprocess',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Closeprojectphaseprocess processing:', error.message);
        throw new Error(`Failed to generate Closeprojectphaseprocess: ${error.message}`);
      } else {
        console.error('Unexpected error in Closeprojectphaseprocess processing:', error);
        throw new Error('An unexpected error occurred while generating Closeprojectphaseprocess');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new CloseprojectphaseprocessTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Closeprojectphaseprocess document.
