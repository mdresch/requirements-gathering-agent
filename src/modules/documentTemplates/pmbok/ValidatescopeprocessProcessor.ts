...
import { ValidatescopeprocessTemplate } from '../pmbok/ValidatescopeprocessTemplate.js';
...
/**
 * Processor for the Validatescopeprocess document.
 */
export class ValidatescopeprocessProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Validatescopeprocess',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Validatescopeprocess processing:', error.message);
        throw new Error(`Failed to generate Validatescopeprocess: ${error.message}`);
      } else {
        console.error('Unexpected error in Validatescopeprocess processing:', error);
        throw new Error('An unexpected error occurred while generating Validatescopeprocess');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new ValidatescopeprocessTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Validatescopeprocess document.
