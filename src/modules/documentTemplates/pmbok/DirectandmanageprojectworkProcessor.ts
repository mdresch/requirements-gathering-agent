...
import { DirectandmanageprojectworkTemplate } from '../pmbok/DirectandmanageprojectworkTemplate.js';
...
/**
 * Processor for the Directandmanageprojectwork document.
 */
export class DirectandmanageprojectworkProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Directandmanageprojectwork',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Directandmanageprojectwork processing:', error.message);
        throw new Error(`Failed to generate Directandmanageprojectwork: ${error.message}`);
      } else {
        console.error('Unexpected error in Directandmanageprojectwork processing:', error);
        throw new Error('An unexpected error occurred while generating Directandmanageprojectwork');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new DirectandmanageprojectworkTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Directandmanageprojectwork document.
