...
import { ProjectmanagementplanTemplate } from '../pmbok/ProjectmanagementplanTemplate.js';
...
/**
 * Processor for the Projectmanagementplan document.
 */
export class ProjectmanagementplanProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Projectmanagementplan',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Projectmanagementplan processing:', error.message);
        throw new Error(`Failed to generate Projectmanagementplan: ${error.message}`);
      } else {
        console.error('Unexpected error in Projectmanagementplan processing:', error);
        throw new Error('An unexpected error occurred while generating Projectmanagementplan');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    return `Based on the following project context, generate a comprehensive Projectmanagementplan document.
