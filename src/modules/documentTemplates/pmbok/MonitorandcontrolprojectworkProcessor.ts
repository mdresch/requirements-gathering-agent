...
import { MonitorandcontrolprojectworkTemplate } from '../pmbok/MonitorandcontrolprojectworkTemplate.js';
...
/**
 * Processor for the Monitorandcontrolprojectwork document.
 */
export class MonitorandcontrolprojectworkProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Monitorandcontrolprojectwork',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Monitorandcontrolprojectwork processing:', error.message);
        throw new Error(`Failed to generate Monitorandcontrolprojectwork: ${error.message}`);
      } else {
        console.error('Unexpected error in Monitorandcontrolprojectwork processing:', error);
        throw new Error('An unexpected error occurred while generating Monitorandcontrolprojectwork');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new MonitorandcontrolprojectworkTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Monitorandcontrolprojectwork document.
