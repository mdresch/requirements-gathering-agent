...
import { QualitymanagementplsnTemplate } from '../planning/QualitymanagementplsnTemplate.js';
...
/**
 * Processor for the Qualitymanagementplsn document.
 */
export class QualitymanagementplsnProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Qualitymanagementplsn',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Qualitymanagementplsn processing:', error.message);
        throw new Error(`Failed to generate Qualitymanagementplsn: ${error.message}`);
      } else {
        console.error('Unexpected error in Qualitymanagementplsn processing:', error);
        throw new Error('An unexpected error occurred while generating Qualitymanagementplsn');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new QualitymanagementplsnTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Qualitymanagementplsn document.
