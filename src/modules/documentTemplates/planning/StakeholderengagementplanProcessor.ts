...
export class StakeholderengagementplanProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Stakeholderengagementplan',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Stakeholderengagementplan processing:', error.message);
        throw new Error(`Failed to generate Stakeholderengagementplan: ${error.message}`);
      } else {
        console.error('Unexpected error in Stakeholderengagementplan processing:', error);
        throw new Error('An unexpected error occurred while generating Stakeholderengagementplan');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new StakeholderengagementplanTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Stakeholderengagementplan document.
