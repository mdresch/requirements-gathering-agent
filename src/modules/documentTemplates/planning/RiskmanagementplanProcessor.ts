...
import { RiskmanagementplanTemplate } from '../planning/RiskmanagementplanTemplate.js';
...
/**
 * Processor for the Riskmanagementplan document.
 */
export class RiskmanagementplanProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Riskmanagementplan processing:', error.message);
        throw new Error(`Failed to generate Riskmanagementplan: ${error.message}`);
      } else {
        console.error('Unexpected error in Riskmanagementplan processing:', error);
        throw new Error('An unexpected error occurred while generating Riskmanagementplan');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    return `Based on the following project context, generate a comprehensive Riskmanagementplan document.
