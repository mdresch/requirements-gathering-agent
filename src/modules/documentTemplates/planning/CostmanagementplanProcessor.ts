...
import { CostmanagementplanTemplate } from '../planning/CostmanagementplanTemplate.js';
...
/**
 * Processor for the Costmanagementplan document.
 */
export class CostmanagementplanProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Costmanagementplan',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Costmanagementplan processing:', error.message);
        throw new Error(`Failed to generate Costmanagementplan: ${error.message}`);
      } else {
        console.error('Unexpected error in Costmanagementplan processing:', error);
        throw new Error('An unexpected error occurred while generating Costmanagementplan');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new CostmanagementplanTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Costmanagementplan document.
