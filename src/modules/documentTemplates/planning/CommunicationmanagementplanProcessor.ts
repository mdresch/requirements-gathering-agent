...
import { CommunicationmanagementplanTemplate } from '../planning/CommunicationmanagementplanTemplate.js';
...
/**
 * Processor for the Communicationmanagementplan document.
 */
export class CommunicationmanagementplanProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Communicationmanagementplan',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Communicationmanagementplan processing:', error.message);
        throw new Error(`Failed to generate Communicationmanagementplan: ${error.message}`);
      } else {
        console.error('Unexpected error in Communicationmanagementplan processing:', error);
        throw new Error('An unexpected error occurred while generating Communicationmanagementplan');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new CommunicationmanagementplanTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Communicationmanagementplan document.
