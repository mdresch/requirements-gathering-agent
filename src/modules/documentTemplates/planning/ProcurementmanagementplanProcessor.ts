...
import { ProcurementmanagementplanTemplate } from '../planning/ProcurementmanagementplanTemplate.js';
...
/**
 * Processor for the Procurementmanagementplan document.
 */
export class ProcurementmanagementplanProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Procurementmanagementplan',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Procurementmanagementplan processing:', error.message);
        throw new Error(`Failed to generate Procurementmanagementplan: ${error.message}`);
      } else {
        console.error('Unexpected error in Procurementmanagementplan processing:', error);
        throw new Error('An unexpected error occurred while generating Procurementmanagementplan');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new ProcurementmanagementplanTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Procurementmanagementplan document.
