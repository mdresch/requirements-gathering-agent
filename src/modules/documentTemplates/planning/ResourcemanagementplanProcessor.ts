...
import { ResourcemanagementplanTemplate } from '../planning/ResourcemanagementplanTemplate.js';
...
/**
 * Processor for the Resourcemanagementplan document.
 */
export class ResourcemanagementplanProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Resourcemanagementplan',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Resourcemanagementplan processing:', error.message);
        throw new Error(`Failed to generate Resourcemanagementplan: ${error.message}`);
      } else {
        console.error('Unexpected error in Resourcemanagementplan processing:', error);
        throw new Error('An unexpected error occurred while generating Resourcemanagementplan');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new ResourcemanagementplanTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Resourcemanagementplan document.
