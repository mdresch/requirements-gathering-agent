...
import { PlanscopemanagementTemplate } from '../planning/PlanscopemanagementTemplate.js';
...
/**
 * Processor for the Plan Scope Management document.
 */
export class PlanscopemanagementProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Plan Scope Management processing:', error.message);
        throw new Error(`Failed to generate Plan Scope Management: ${error.message}`);
      } else {
        console.error('Unexpected error in Plan Scope Management processing:', error);
        throw new Error('An unexpected error occurred while generating Plan Scope Management');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new PlanscopemanagementTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Plan Scope Management document.
...
- Keep content concise but comprehensive`;
  ...
  getDescription(): string {
    return 'Generates a comprehensive Plan Scope Management document following PMBOK guidelines';
  }
