...
import { ScopebaselineTemplate } from '../scope-management/ScopebaselineTemplate.js';
...
/**
 * Processor for the Scope Baseline document.
 */
export class ScopebaselineProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Scope Baseline processing:', error.message);
        throw new Error(`Failed to generate Scope Baseline: ${error.message}`);
      } else {
        console.error('Unexpected error in Scope Baseline processing:', error);
        throw new Error('An unexpected error occurred while generating Scope Baseline');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new ScopebaselineTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Scope Baseline document.
...
- Keep content concise but comprehensive`;
  ...
  getDescription(): string {
    return 'Generates a comprehensive Scope Baseline document following PMBOK guidelines';
  }
