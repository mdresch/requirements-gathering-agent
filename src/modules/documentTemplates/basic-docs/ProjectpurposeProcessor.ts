...
import { ProjectpurposeTemplate } from '../basic-docs/ProjectpurposeTemplate.js';
...
/**
 * Processor for the Projectpurpose document.
 */
export class ProjectpurposeProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Projectpurpose',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Projectpurpose processing:', error.message);
        throw new Error(`Failed to generate Projectpurpose: ${error.message}`);
      } else {
        console.error('Unexpected error in Projectpurpose processing:', error);
        throw new Error('An unexpected error occurred while generating Projectpurpose');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new ProjectpurposeTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Projectpurpose document.
