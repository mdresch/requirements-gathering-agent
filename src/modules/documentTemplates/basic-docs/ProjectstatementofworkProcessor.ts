...
import { ProjectstatementofworkTemplate } from '../basic-docs/ProjectstatementofworkTemplate.js';
...
/**
 * Processor for the Projectstatementofwork document.
 */
export class ProjectstatementofworkProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Projectstatementofwork',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Projectstatementofwork processing:', error.message);
        throw new Error(`Failed to generate Projectstatementofwork: ${error.message}`);
      } else {
        console.error('Unexpected error in Projectstatementofwork processing:', error);
        throw new Error('An unexpected error occurred while generating Projectstatementofwork');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new ProjectstatementofworkTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Projectstatementofwork document.
