...
export class PersonasassessmotivationsProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Personasassessmotivations',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Personasassessmotivations processing:', error.message);
        throw new Error(`Failed to generate Personasassessmotivations: ${error.message}`);
      } else {
        console.error('Unexpected error in Personasassessmotivations processing:', error);
        throw new Error('An unexpected error occurred while generating Personasassessmotivations');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new PersonasassessmotivationsTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Personasassessmotivations document.
