...
import { TechnologycomfortuserpersonasTemplate } from '../basic-docs/TechnologycomfortuserpersonasTemplate.js';
...
/**
 * Processor for the Technologycomfortuserpersonas document.
 */
export class TechnologycomfortuserpersonasProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Technologycomfortuserpersonas',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Technologycomfortuserpersonas processing:', error.message);
        throw new Error(`Failed to generate Technologycomfortuserpersonas: ${error.message}`);
      } else {
        console.error('Unexpected error in Technologycomfortuserpersonas processing:', error);
        throw new Error('An unexpected error occurred while generating Technologycomfortuserpersonas');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new TechnologycomfortuserpersonasTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Technologycomfortuserpersonas document.
