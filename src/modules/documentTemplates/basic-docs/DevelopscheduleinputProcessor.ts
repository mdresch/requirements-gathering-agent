...
import { DevelopscheduleinputTemplate } from '../basic-docs/DevelopscheduleinputTemplate.js';
...
/**
 * Processor for the Developscheduleinput document.
 */
export class DevelopscheduleinputProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Developscheduleinput',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Developscheduleinput processing:', error.message);
        throw new Error(`Failed to generate Developscheduleinput: ${error.message}`);
      } else {
        console.error('Unexpected error in Developscheduleinput processing:', error);
        throw new Error('An unexpected error occurred while generating Developscheduleinput');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new DevelopscheduleinputTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Developscheduleinput document.
