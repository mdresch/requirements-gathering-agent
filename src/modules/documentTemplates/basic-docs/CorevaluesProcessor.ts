...
import { CorevaluesTemplate } from '../basic-docs/CorevaluesTemplate.js';
...
/**
 * Processor for the Corevalues document.
 */
export class CorevaluesProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Corevalues processing:', error.message);
        throw new Error(`Failed to generate Corevalues: ${error.message}`);
      } else {
        console.error('Unexpected error in Corevalues processing:', error);
        throw new Error('An unexpected error occurred while generating Corevalues');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new CorevaluesTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Corevalues document.
