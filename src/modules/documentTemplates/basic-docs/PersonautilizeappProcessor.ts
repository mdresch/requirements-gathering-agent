...
import { PersonautilizeappTemplate } from '../basic-docs/PersonautilizeappTemplate.js';
...
/**
 * Processor for the Personautilizeapp document.
 */
export class PersonautilizeappProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      return {
        title: 'Personautilizeapp',
        content
      };
    } catch (error) {
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Personautilizeapp processing:', error.message);
        throw new Error(`Failed to generate Personautilizeapp: ${error.message}`);
      } else {
        console.error('Unexpected error in Personautilizeapp processing:', error);
        throw new Error('An unexpected error occurred while generating Personautilizeapp');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new PersonautilizeappTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Personautilizeapp document.
