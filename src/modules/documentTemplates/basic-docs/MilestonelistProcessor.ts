...
import { MilestonelistTemplate } from '../basic-docs/MilestonelistTemplate.js';
...
/**
 * Processor for the Milestonelist document.
 */
export class MilestonelistProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Milestonelist processing:', error.message);
        throw new Error(`Failed to generate Milestonelist: ${error.message}`);
      } else {
        console.error('Unexpected error in Milestonelist processing:', error);
        throw new Error('An unexpected error occurred while generating Milestonelist');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new MilestonelistTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Milestonelist document.
