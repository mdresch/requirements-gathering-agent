...
import { RiskanalysisTemplate } from '../basic-docs/RiskanalysisTemplate.js';
...
/**
 * Processor for the Riskanalysis document.
 */
export class RiskanalysisProcessor implements DocumentProcessor {
  ...
  async process(context: ProjectContext): Promise<DocumentOutput> {
    try {
      ...
      if (error instanceof ExpectedError) {
        console.warn('Expected error in Riskanalysis processing:', error.message);
        throw new Error(`Failed to generate Riskanalysis: ${error.message}`);
      } else {
        console.error('Unexpected error in Riskanalysis processing:', error);
        throw new Error('An unexpected error occurred while generating Riskanalysis');
      }
    }
  }
  ...
  private createPrompt(context: ProjectContext): string {
    ...
    const template = new RiskanalysisTemplate(context);
    const exampleStructure = template.generateContent();

    return `Based on the following project context, generate a comprehensive Riskanalysis document.
