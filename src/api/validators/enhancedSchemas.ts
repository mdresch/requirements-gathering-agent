import Joi from 'joi';
import { VALIDATION_RULES } from '../../models/index.js';

/**
 * Enhanced Validation Schemas
 * 
 * Comprehensive validation schemas that use the common validation rules
 * from the models index. Ensures consistency across all API endpoints.
 */
...
export const templateSchemas = {
  create: {
    body: Joi.object({
      ...
      contextPriority: commonValidations.priority,
  ...
  list: {
    query: commonValidations.pagination.keys({
...
export const projectSchemas = {
  create: {
    body: Joi.object({
      ...
      startDate: commonValidations.date.required().messages({
        'any.required': 'Start date is required'
      }),
      
      endDate: commonValidations.date.optional(),
      
      complianceScore: commonValidations.percentage,
      ...
      priority: commonValidations.priority,
  ...
  list: {
    query: commonValidations.pagination.keys({
      ...
      owner: Joi.string().trim().max(100).optional(),
      startDateFrom: commonValidations.date.optional(),
      startDateTo: commonValidations.date.optional()
    })
  }
};
...
export const feedbackSchemas = {
  create: {
    body: Joi.object({
      documentId: commonValidations.objectId.messages({
        'any.required': 'Document ID is required'
      }),
      
      projectId: commonValidations.objectId.messages({
  ...
  update: {
    body: Joi.object({
      ...
      resolvedAt: commonValidations.date.optional(),
  ...
  list: {
    query: commonValidations.pagination.keys({
      search: Joi.string().trim().max(100).optional(),
      projectId: commonValidations.objectId.optional(),
      documentId: commonValidations.objectId.optional(),
      feedbackType: Joi.string().valid('general', 'technical', 'content', 'structure', 'compliance', 'quality').optional(),
      category: Joi.string().valid('positive', 'negative', 'suggestion', 'question', 'issue').optional(),
      status: Joi.string().valid('open', 'in_review', 'addressed', 'resolved', 'rejected', 'closed').optional(),
      priority: commonValidations.priority,
...
export const auditTrailSchemas = {
  create: {
    body: Joi.object({
      ...
      userName: Joi.string().trim().max(100).optional(),
      userEmail: commonValidations.email.optional(),
      ...
      severity: commonValidations.severity,
  ...
  list: {
    query: commonValidations.pagination.keys({
      entityType: Joi.string().valid('template', 'project', 'category', 'feedback', 'stakeholder', 'document', 'user', 'system').optional(),
      entityId: Joi.string().trim().max(100).optional(),
      action: Joi.string().valid('created', 'updated', 'deleted', 'restored', 'activated', 'deactivated', 'assigned', 'unassigned', 'status_changed', 'permission_changed', 'login', 'logout', 'export', 'import').optional(),
      userId: Joi.string().trim().max(100).optional(),
      projectId: Joi.string().trim().max(100).optional(),
      documentId: Joi.string().trim().max(100).optional(),
      severity: commonValidations.severity,
      category: Joi.string().valid('data_change', 'access', 'system', 'security', 'compliance', 'workflow').optional(),
      complianceFramework: Joi.string().valid('sox', 'gdpr', 'hipaa', 'pci', 'iso27001', 'custom').optional(),
      requiresReview: Joi.boolean().optional(),
      dateFrom: commonValidations.date.optional(),
      dateTo: commonValidations.date.optional()
    })
  }
};
