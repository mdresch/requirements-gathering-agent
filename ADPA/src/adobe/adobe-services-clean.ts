...
class AdobeCredentialManager {
  ...
  constructor() {
    const keyvaultUrl = typeof process !== "undefined" && process.env?.AZURE_KEYVAULT_URL 
...
`;
  ...
  private async generateChart(chart: ChartData, branding: BrandingConfig): Promise<string> {
    ...
    return `<svg><!-- Generated chart for ${chart.title} --></svg>`;
  }
  ...
  private async uploadDocument(buffer: Buffer, title: string, format: string): Promise<string> {
    const connectionString = typeof process !== "undefined" && process.env?.AZURE_STORAGE_CONNECTION_STRING
