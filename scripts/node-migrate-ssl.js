...
/**
 * Node.js MongoDB Migration Script with SSL handling
 * 
 * This script migrates data from Docker MongoDB to Atlas using Node.js MongoDB driver
 * with enhanced SSL/TLS handling for Windows environments.
 */
...
class NodeMigrationSSL {
  ...
  async connectToAtlas() {
    this.log('Connecting to MongoDB Atlas with enhanced SSL handling...');
    
    // Try multiple connection options for SSL issues
    const connectionOptions = [
      ...
      // Option 3: With relaxed SSL options (for testing)
      {
        name: 'With relaxed SSL',
        options: {
          tls: true,
      ...
      // Option 4: With connection timeout
      {
        name: 'With timeout',
        options: {
  ...
  async exportDataToFiles() {
    ...
    for (const collectionName of COLLECTIONS) {
      try {
        ...
        this.log(`Exported ${documents.length} documents from ${collectionName} to ${filename}`, 'success');
      } catch (error) {
...
`;
  ...
  async run() {
    try {
      this.log('Starting Node.js MongoDB Migration to Atlas with SSL handling', 'info');
...
// Run migration
const migration = new NodeMigrationSSL();
